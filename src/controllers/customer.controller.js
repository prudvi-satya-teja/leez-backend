...
const signup = async (req, res) => {
    try {
        ...
        if(user.length > 0) {
            return res.status(400).json({success: true, message: "User already exists with email or password"});
        }

        const otp = otpGenerator.generate(4, {specialChars: false});
        const userOtpObject = new otp({user: user, otp: otp});
        userOtpObject.save();
        sendMail(email, "account creation", `Your otp for account creation in our Leez account is ${otp}. It is valid only for 10 minutes.`);
...
const createAccount = async (req, res) => {
    try {
        ...
        const customerObject = new Customer({email: email, phoneNo: phoneNo, name: name, password: password});
